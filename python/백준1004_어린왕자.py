...
for j in range(T):
    ...
    tmp = 0
    for i in range(n):
        cx[i], cy[i], r[i] = map(int, input().split())
    for i in range(n):
        if ((x1-cx[i])*(x1-cx[i])+(y1-cy[i])*(y1-cy[i]) > r[i]*r[i]) and ((x2-cx[i])*(x2-cx[i])+(y2-cy[i])*(y2-cy[i]) > r[i]*r[i]):
            pass
        elif (x1-cx[i])*(x1-cx[i])+(y1-cy[i])*(y1-cy[i]) > r[i]*r[i]:
            tmp += 1
        elif (x2-cx[i])*(x2-cx[i])+(y2-cy[i])*(y2-cy[i]) > r[i]*r[i]:
            tmp += 1
        else:
            pass
    ans.append(tmp)
